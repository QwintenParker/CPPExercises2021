...
void test3Top2ElementSearch() {
    ...
    long long maX1 = MIN_INT;
    long long maX2 = MIN_INT;
    t.restart();
    #pragma omp parallel
    {
        // TODO сделайте многопоточную версию (используйте самописную редукцию)
        long long max1Thread = MIN_INT;
        long long max2Thread = MIN_INT;
        #pragma omp for
        for (int i = 0; i < n; ++i) {
            int value = data[i];
            if (value > max1Thread) { // если текущее значение больше самого большого
                max2Thread = max1Thread;    // то число которое раньше было самым большим - становится вторым по величине
                max1Thread = value;   // а текущее значение становится самым большим
            } else {            // иначе:
                if (value > max2Thread) { // если текущее значение больше хотя бы второго по величине
                    max2Thread = value;   // то заменяем его
                }
            }
        }
        #pragma omp critical
...
void test4HowWorkloadIsBalanced() {
    std::cout << "Test 4: how work load is balanced..." << std::endl;
